...
int main(int argc, char *argv[]) {
    ...
    // Multiplicacion de matrices, columna por fila - paralelizada
    omp_set_num_threads(num_threads);
    #pragma omp parallel for collapse(2)
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            for (int k = 0; k < cols; k++) {
                mult[i][j] += matrix[i][k] * m2[k][j];
            }
        }
    }
    ...
    for (int i = 0; i < rows; i++) {
        ...
        delete[] m2[i]; //libera memoria de cada fila
        delete[] mult[i]; //libera memoria de cada fila
    }
    delete[] matrix; //libera la memoria de los punteros en matrix
    delete[] m2; //libera la memoria de los punteros en matrix
    delete[] mult; //libera la memoria de los punteros en matrix
